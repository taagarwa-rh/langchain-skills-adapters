...
class TestSkillModel:
    def test_required_fields(self, make_skill):
        skill = make_skill()
    ...
    def test_optional_fields_default_to_none(self, make_skill):
        skill = make_skill()
        assert skill.license is None
        assert skill.compatibility is None
        assert skill.metadata is None

    def test_allowed_tools_defaults_to_empty_list(self, make_skill):
        skill = make_skill()
        assert skill.allowed_tools == []

    def test_resources_defaults_to_empty_list(self, make_skill):
        skill = make_skill()
        assert skill.resources == []

    def test_optional_fields_can_be_set(self, make_skill):
        skill = make_skill(
            license="MIT",
            compatibility=">=1.0",
...
class TestSkillToCatalog:
    def test_contains_skill_xml_tags(self, make_skill):
        result = make_skill().to_catalog()
        assert "<skill>" in result
        assert "</skill>" in result

    def test_contains_name(self, make_skill):
        result = make_skill(name="my-skill").to_catalog()
        assert "<name>my-skill</name>" in result

    def test_contains_description(self, make_skill):
        result = make_skill(description="Summarize text.").to_catalog()
        assert "<description>Summarize text.</description>" in result

    def test_contains_location(self, make_skill):
        loc = Path("/skills/my-skill/SKILL.md")
        result = make_skill(location=loc).to_catalog()
        assert f"<location>{loc}</location>" in result

    def test_does_not_contain_content(self, make_skill):
        result = make_skill(content="secret content").to_catalog()
        assert "secret content" not in result


class TestSkillToContent:
    def test_contains_skill_content_tag_with_name(self, make_skill):
        result = make_skill(name="my-skill").to_content()
        assert '<skill_content name="my-skill">' in result
        assert "</skill_content>" in result

    def test_contains_content_body(self, make_skill):
        result = make_skill(content="Do the thing.").to_content()
        assert "Do the thing." in result

    def test_contains_skill_directory(self, make_skill):
        skill = make_skill(location=Path("/skills/test-skill/SKILL.md"))
        result = skill.to_content()
        assert f"Skill directory: {Path('/skills/test-skill')}" in result

    def test_no_resources_omits_skill_resources_section(self, make_skill):
        result = make_skill().to_content()
        assert "<skill_resources>" not in result
        assert "</skill_resources>" not in result

    def test_contains_skill_resources_section_when_resources_present(self, make_skill, tmp_path):
        skill_dir = tmp_path / "my-skill"
        skill_dir.mkdir()
        location = skill_dir / "SKILL.md"
        resource = skill_dir / "data" / "ref.txt"

        skill = make_skill(location=location, resources=[resource])
        result = skill.to_content()
        assert "<skill_resources>" in result
        assert "</skill_resources>" in result

    def test_lists_resource_files_relative_to_skill_dir(self, make_skill, tmp_path):
        skill_dir = tmp_path / "my-skill"
        skill_dir.mkdir()
        location = skill_dir / "SKILL.md"
        resource = skill_dir / "data" / "ref.txt"
    ...
    def test_multiple_resources_listed(self, make_skill, tmp_path):
        skill_dir = tmp_path / "my-skill"
        skill_dir.mkdir()
        location = skill_dir / "SKILL.md"
        resources = [skill_dir / "a.txt", skill_dir / "b.txt"]

        skill = make_skill(location=location, resources=resources)
        result = skill.to_content()
...
class TestSkillCatalogModel:
    ...
    def test_accepts_list_of_skills(self, make_skill):
        skills = [make_skill(name="a"), make_skill(name="b")]
...
class TestSkillCatalogToStr:
    ...
    def test_contains_available_skills_tags(self, make_skill):
        catalog = SkillCatalog(skills=[make_skill()])
        result = catalog.to_str()
        assert "<available_skills>" in result
        assert "</available_skills>" in result

    def test_contains_skill_entries(self, make_skill):
        catalog = SkillCatalog(skills=[make_skill(name="my-skill")])
        result = catalog.to_str()
        assert "<name>my-skill</name>" in result

    def test_multiple_skills_all_appear(self, make_skill):
        skills = [make_skill(name="alpha"), make_skill(name="beta")]
        catalog = SkillCatalog(skills=skills)
        result = catalog.to_str()
        assert "<name>alpha</name>" in result
        assert "<name>beta</name>" in result

    def test_entries_are_indented(self, make_skill):
        catalog = SkillCatalog(skills=[make_skill()])
        result = catalog.to_str()
